  ...
  return (
    <div className="flex h-screen flex-col">
      <header className="flex items-center gap-4 border-b border-neutral-800 px-4 py-2.5">
        ...
        <span className="text-sm font-semibold">Редактор лекал</span>
        <input
          ref={fileRef}
          type="file"
          ...
          <aside className="w-80 shrink-0 overflow-y-auto border-l border-neutral-800 bg-neutral-900 p-4 text-sm">
            ...
            <Section title="Экспорт">
              <div className="mb-2 flex gap-2">
                <button onClick={download} className="flex-1 rounded bg-neutral-700 px-2 py-1.5 text-xs hover:bg-neutral-600">
                  Скачать JSON
                </button>
                <button onClick={() => navigator.clipboard?.writeText(json)} className="flex-1 rounded bg-neutral-700 px-2 py-1.5 text-xs hover:bg-neutral-600">
                  Копировать
                </button>
              </div>
              <button
              ...
              >
                Добавить в каталог (черновик)
              </button>
              {saved && (
                <p className="mt-2 text-xs text-emerald-400">
                  «{saved}» в каталоге. <Link href="/" className="underline">Открыть в редакторе →</Link>
                </p>
              )}
              <p className="mt-2 text-xs text-neutral-500">
                Черновик виден в каталоге (превью). Для прод/PDF — сохрани флэт в
                <code className="mx-1">public/seed/flats/</code> и впиши JSON в
                <code className="mx-1">skus.json</code>.
              </p>
...
const inp =
  "w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5";
